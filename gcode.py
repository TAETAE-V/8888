...
def parseGCode(lines):
    ...
    for i in range(len(lines)):
        if len(lines[i]) == 0:
            continue
        if lines[i].startswith(";LAYER:1") and lines[i].endswith(":1"):
            stop = True

        if lines[i].startswith(';'):  # comment
            if lines[i].startswith(";LAYER:"):
                finishLayer()
            elif lines[i].startswith(";End") or lines[i].startswith("; z_offset"):
                break
        if lines[i].startswith(";LAYER:"):
            floor +=1
        ...
        if stop is True:
            ...
            if "G1" in lines[i] or "G0" in lines[i] and "Move" not in lines[i]:
                a = float(getValue(lines[i], "X", -1))
                b = float(getValue(lines[i], "Y", -1))
                if softChose == True:
                    c = float(getZ(lines[i],-1))
                    if c != -1:
                        newC = c
                        print(newC)
                    else:
                    ...
                    c = float(getValue(lines[i],"Z",-1))
                    newC = c
                    #print(c)
                if a != -1 and b != -1 and newC != -1:
                    if len(plane) < 1:
                        plane.append([a, -b,-newC])
                    #print(plane)
            ...
            if args[0] == "G0":  # move to (or rotate)
                ...
                if z_rot is not None:
                 ...
                 divide.append(floor)
                 a = float(getValue(lines[i], "A", -1))
                 b = float(getValue(lines[i], "B", -1))
                 c = float(getValue(lines[i], "C", -1))
    ...
    divide.append((floor))
    print(divide,floor,color,center)
...
def getZ(line,default = None):
    if "Z" in line and "F" in line and "nozzle" not in line:
        subPart = line[(line.find("Z") + 1):]
        m = re.search("[0-9]*\.[0-9]*",subPart)
        if m == None:
            return default
        try:
